...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){
			...
			if (max_average < student.average) {
				if (max_load <= student.load) {
					max_average = student.average;
					max_load = student.load;
				}
			}
		}
		
		printf("%d, %f", max_load, max_average);
		for (int i = 0; i < size; ++i){ // process all the student records in database
			Student s = students[i]; // store data for each student in s
			
			if( ((max_average - s.average <= 1) && (s.load == max_load)) && !((max_average == s.average) && (s.load == max_load)) ){ // *** first filter, conditions on the student
