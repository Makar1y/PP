	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(s.load){ // *** first filter, conditions on the student
				...
				if (s.average != true_average) {// *** third filter, various other conditions	
					printf("True average %f != current average %f\n", true_average, s.average);	
