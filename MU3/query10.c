...
int lang_check(char lang[]) {
	char lithuanian[] = "Lithuanian";
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(lang_check(s.languages)){ // *** first filter, conditions on the student
				printf("%s %s ", s.name, s.surname); // print student data	
		...
		printf("\n\nFilter applied, %d students found", counterDemo); // how many passed the filters
		fclose(db);	
	} else {
